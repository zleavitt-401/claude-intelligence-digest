...
If you find no relevant news from the last 24 hours, return an empty array: \`\`\`json\n[]\n\`\`\``,
    ...
    if (!response.ok) {
      ...
      research._lastRawResponse = { httpStatus: response.status, error: errText }; // DEBUG
      return [];
    }

    const data = await response.json();

    // DEBUG: log the full raw response so we can see what Claude actually returned
    console.log('[researcher:debug] stop_reason:', data.stop_reason);
    console.log('[researcher:debug] content block types:', (data.content || []).map((b) => b.type));
    console.log('[researcher:debug] full response:', JSON.stringify(data, null, 2));

    // Extract text content blocks from the response
    const textBlocks = (data.content || []).filter((block) => block.type === 'text');
    const fullText = textBlocks.map((block) => block.text).join('\n');

    console.log('[researcher:debug] extracted fullText:', fullText);
    ...
    // DEBUG: return items AND raw response via a module-level variable so cron.js can include it
    research._lastRawResponse = { stop_reason: data.stop_reason, contentTypes: (data.content || []).map((b) => b.type), content: data.content };

    return items;
  } catch (err) {
    console.error('[researcher] Research failed:', err.message || err);
    research._lastRawResponse = { error: err.message || String(err) }; // DEBUG
    return [];
  }
}
