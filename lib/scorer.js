...
const SCORING_MODEL = 'claude-sonnet-4-5';
...
Published: ${item.publishedAt}`,
    ...
    if (!response.ok) {
      ...
      return [{ _error: `HTTP ${response.status}: ${errText}` }]; // DEBUG
    }
    ...
    return [{ _error: err.message || String(err) }]; // DEBUG
  }
}
...
async function scoreItems(rawItems, config) {
  ...
  const digestItems = [];
  const allScoredItems = []; // DEBUG

  // Score items sequentially to respect rate limits
  for (const item of rawItems) {
    const scores = await scoreOneItem(item, systemPrompt, apiKey);
    if (scores.length === 0 || scores[0]?._error) {
      allScoredItems.push({ headline: item.headline, maxScore: null, scores, filtered: 'no-scores' }); // DEBUG
      continue;
    }

    const maxScore = Math.max(...scores.map((s) => s.score));
    const passesThreshold = maxScore >= threshold;

    // DEBUG: log every item and its scores
    console.log(`[scorer:debug] "${item.headline}" | maxScore=${maxScore} | passes=${passesThreshold} | scores=${JSON.stringify(scores)}`);
    allScoredItems.push({ headline: item.headline, maxScore, scores, filtered: passesThreshold ? 'kept' : 'below-threshold' }); // DEBUG
  ...
  return { items: digestItems, _debugAll: allScoredItems, threshold }; // DEBUG: return object
}
